...
(()=>{
  ...
  function renderFeed(items){if(!feed||!items?.length)return;feed.innerHTML=items.slice(0,8).map((x,i)=>`<article class="card post news-card ${i===0?'news-featured':''}">${x.image?`<img class="news-image" src="${esc(x.image)}" alt="" loading="lazy">`:''}<div class="news-meta"><span class="news-source">${esc(x.source||'Fuente académica')}</span><span>${esc(x.date||'Reciente')}</span></div><h3>${esc(x.title)}</h3><p>${esc(x.summary||'Información reciente relacionada con tus materias.')}</p><div class="news-footer">${x.url?`<a class="news-link" href="${esc(x.url)}" target="_blank" rel="noopener">Leer artículo ↗</a>`:''}<span class="news-topic">Actualidad</span></div></article>`).join('')}
