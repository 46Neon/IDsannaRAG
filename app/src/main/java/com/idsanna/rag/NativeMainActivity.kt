...
class NativeMainActivity : AppCompatActivity() {
    ...
    private fun navigation():HorizontalScrollView{val row=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL;setPadding(4,8,4,8)};listOf("⌂  Inicio","▦  Materias","◌  Chat","●  Perfil").forEachIndexed{index,label->row.addView(button(label){screenIndex=index;when(index){0->showHome();1->showSubjects();2->showChat();else->showProfile()}},LinearLayout.LayoutParams(150,56))};return HorizontalScrollView(this).apply{isHorizontalScrollBarEnabled=false;setBackgroundColor(Color.rgb(13,27,46));addView(row)}}
