...
class NativeMainActivity : AppCompatActivity() {
    ...
    private fun show(view:View){content.removeAllViews();content.addView(view,FrameLayout.LayoutParams(-1,-1));content.requestFocus()}
    ...
    private fun navigation()=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.BOTTOM;setPadding(0,16,0,0);addView(button("Inicio"){showHome()},LinearLayout.LayoutParams(0,56,1f));addView(button("Materias"){showSubjects()},LinearLayout.LayoutParams(0,56,1f));addView(button("Chat"){showChat()},LinearLayout.LayoutParams(0,56,1f));addView(button("Perfil"){showProfile()},LinearLayout.LayoutParams(0,56,1f))}
