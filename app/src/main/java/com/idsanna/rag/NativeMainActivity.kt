...
class NativeMainActivity : AppCompatActivity() {
    ...
    private fun navButton(label:String,index:Int)=MaterialButton(this).apply{this.text=label;isAllCaps=false;textSize=20f;setTextColor(if(index==screenIndex)bg else white);setBackgroundColor(if(index==screenIndex)teal else Color.TRANSPARENT);cornerRadius=22;setPadding(8,0,8,0);setOnClickListener{screenIndex=index;when(index){0->showHome();1->showSubjects();2->showChat();else->showProfile()}}}
    private fun navigation():HorizontalScrollView{val row=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL;setPadding(8,8,8,8)};listOf("⌂","▦","◌","●").forEachIndexed{index,label->row.addView(navButton(label,index),LinearLayout.LayoutParams(0,62,1f))};return HorizontalScrollView(this).apply{isHorizontalScrollBarEnabled=false;setBackgroundColor(Color.rgb(13,27,46));addView(row)}}
    private fun field(hint:String,password:Boolean=false)=TextInputLayout(this).apply{hint=hint;setBoxBackgroundMode(TextInputLayout.BOX_BACKGROUND_OUTLINE);setBoxCornerRadii(12f,12f,12f,12f);val e=TextInputEditText(this@NativeMainActivity);e.setTextColor(white);e.setHintTextColor(muted);if(password)e.inputType=0x81;addView(e)}
