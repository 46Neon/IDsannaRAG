...
Deno.serve(async (request) => {
  ...
  let input: { subject_id?: string; conversation_id?: string; message?: string; idempotency_key?: string };
  ...
  const provider = configuredProvider(); if (!provider) return json({ error: 'provider_not_configured' }, 503);
  ...
  try {
    const answer = await provider.complete({ system: `Eres IDsanna, asistente académico. Responde en español con evidencia y reconoce incertidumbre. Materia: ${subject.data.name}.`, user: message });
